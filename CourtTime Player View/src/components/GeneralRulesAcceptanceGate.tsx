...
export function GeneralRulesAcceptanceGate() {
  const { pendingGeneralRulesAcceptances, acceptGeneralRulesAndContinue } = useAuth();
  ...
  const handleScrollRules = (e: React.UIEvent<HTMLDivElement>) => {
  ...
  const handleAccept = async () => {
    ...
    setSubmitting(true);
    const ok = await acceptGeneralRulesAndContinue(current.facilityId);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">General Rules Update Required</CardTitle>
            <p className="text-sm text-gray-600">
              You must accept the latest General Rules for <strong>{current.facilityName}</strong> before you can book a court or continue using the club.
            </p>
          ...
          <CardContent className="space-y-4">
            ...
            {!scrolledToBottom && (
              <p className="text-xs text-gray-500">
                Scroll to the bottom of the rules to enable acceptance.
              </p>
            )}

            <div className="flex items-start space-x-3">
              <Checkbox
                id="general-rules-agree"
                checked={agreed}
                onCheckedChange={(checked) => setAgreed(Boolean(checked))}
                disabled={!scrolledToBottom}
              />
              <label htmlFor="general-rules-agree" className="text-sm leading-5">
                I have read and agree to the General Rules
              </label>
            </div>
