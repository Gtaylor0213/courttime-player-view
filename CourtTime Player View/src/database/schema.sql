...
CREATE TABLE bulletin_drill_signups (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_bulletin_drill_signups_post ON bulletin_drill_signups(bulletin_post_id, status, created_at);
CREATE INDEX idx_bulletin_drill_signups_waitlist ON bulletin_drill_signups(bulletin_post_id, waitlist_position) WHERE status = 'waitlist';
...
CREATE TRIGGER update_bulletin_drill_signups_updated_at BEFORE UPDATE ON bulletin_drill_signups FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
